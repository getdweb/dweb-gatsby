...
let onPreBootstrap = async () => {
  ...
  data_list.forEach((data_file) => {
    DATA_ARR.push([
      ...
      `${data_file.slice(0, -5)}--index.yaml`,
    ])
    fs.copyFileSync(
      `./src/data/${data_file}`,
      `./src/data/${data_file.slice(0, -5)}--index.yaml`
...
let createPages = ({ actions, graphql }) => {
  ...
  page_list.forEach((page_name) => {
    let page_data = yaml.load(
      fs.readFileSync(`./static/page-data/${page_name}`, 'utf8')
    )
...
let onPostBuild = async () => {
  ...
  await pMap(
    ...
    async (path) => {
      if (
        ...
        contents = contents.replace(/\/images\//g, () => './images/')
  ...
  for (const hash of indexHashArray) {
    const indexHashDataBuffer = await readFileAsync(
      `./public/page-data/sq/d/${hash}.json`
    )
    let indexHashData = indexHashDataBuffer.toString()

    indexHashData = indexHashData.replace(/\.\.\/images\//g, () => './images/')
