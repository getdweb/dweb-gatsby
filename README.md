...
1. There are seven unique modular components any given page might use. The structure of a page is dictated by an array of components given in the page's YAML file. For example, the Contact page is defined by the [Contact.yaml](static/page-data/Contact) file which invokes [PageBlockOpening](src/components/PageBlockOpening.js) followed by [PageBlockFeature](src/components/PageBlockFeature.js). Beneath each component key is the relevant information each component needs to function properly. [Below](#modular-components-and-their-properties) is a complete list of modules and their properties:
    - In order to avoid the ambiguity of duplicate keys in YAML, component data is stored in a YAML array. For example, this is necessary in [dweb-camp-2022.yaml](static/page-data/dweb-camp-2022.yaml) where there are multiple instances of `PageBlockFeature`.
...
1. Links in [Menu.yaml](src/data/Menu.yaml) under the label `secondary` must have a closing forward slash. For example, correct formatting looks like `url: /principles/` and NOT `url: /principles`. Failure to comply will break the site feature that highlights the current page if it is linked on the navigation menu.
1. URLs to all local images should be made as if from within the static folder. In other words, all image URLs should be of the format `/images/example.jpeg`. The relativization of these links is done during the build process.
...
- [X] [Minified React error #418](https://reactjs.org/docs/error-decoder.html?invariant=418) and [Minified React error #423](https://reactjs.org/docs/error-decoder.html?invariant=423) began appearing in site build after upgrading to Gatsby v5 and React v18. The errors do not appear to affect site funcitonality, and are most likely caused by references to the global `window` as described [here](https://github.com/gatsbyjs/gatsby/discussions/36232#discussioncomment-3229914).
- [X] The video element on the frontpage does not appear to be loading the video properly, it appears to be a change on the Internet Archive's end because previous version's of the site cannot load it either.
- [X] In [Principles.yaml](src/data/Principles.yaml), the YAML is storing raw HTML when it should instead lists of the principles using standard YAML syntax. Be careful when fixing this because the python transition tools output this HTML-formatted list. It would make sense to update Principles.yaml and the associated component only after the final break from Wordpress is made so that the transition tools do not interfere.
- [X] The link to the video and background link in [Hero.js](src/components/Hero.js) should live in a YAML file inside the [data](src/data/) folder.
